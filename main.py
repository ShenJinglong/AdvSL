...
if config.disc_type == "anchor":
    ...
    cluster_unions = [ClusterUnionAnchor(config.disc_mode, config.k, fm_size, config.lr, DEVICE) for _ in range(num_client-1)]
...
#######################################################################################################################################################################
                ...
                if config.disc_mode == "gr": # 如果鉴别器是梯度反转模式，则将各个client model上的梯度乘-1进行反转
                    ...
                    if config.ratio_disc < 0: # 此时如果ratio_disc是负数，会抵消梯度反转的效果
                        logging.warning("`ratio_disc` is negative while discriminator is worked in gradient reverse mode.")
                # 控制鉴别器反向回来的梯度的大小
